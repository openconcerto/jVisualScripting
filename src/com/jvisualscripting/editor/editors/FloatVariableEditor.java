...
public class FloatVariableEditor implements NodeEditor {
    ...
    public JPanel createEditor(EventGraphEditorPanel panel, Node n) {
        ...
        c.weightx = 1;
        final FloatVariable v = (FloatVariable) n;
        ...
        final JSpinner text = new JSpinner(new SpinnerNumberModel((Float) v.getValue(), (Float) (-Float.MAX_VALUE), (Float) Float.MAX_VALUE, (Float) 1.5f));
        p.add(text, c);
        ...
        final SwingThrottle t = new SwingThrottle(100, new Runnable() {
            ...
            @Override
            public void run() {
                System.out.println("FloatVariableEditor.createEditor(...).new Runnable() {...}.run()");

                v.setValue(((Float) text.getValue()));
