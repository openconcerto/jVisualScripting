...
public abstract class MultipleOutputNodeEditor implements NodeEditor {
    ...
    public JPanel createEditor(EventGraphEditorPanel panel, Node n) {
        ...
        final JSpinner spinner = new JSpinner(new SpinnerNumberModel(n.getOutputSize(), 2, 256, 1));
        p.add(spinner, c);
        ...
        final SwingThrottle t = new SwingThrottle(100, new Runnable() {
            ...
            public void run() {
                ...
                if (n.getOutputSize() > newSize) {
                    int toRemove = n.getOutputSize() - newSize;
                    for (int i = 0; i < toRemove; i++) {
                        Pin pinToRemove = n.getLastOutputPin();
                        VLink link = panel.getVLink(pinToRemove);
                        if (link != null) {
                            panel.remove(link);
                        }
                        n.getOutputs().remove(pinToRemove);
                    }

                } else if (n.getOutputSize() < newSize) {
                    int toAdd = newSize - n.getOutputSize();
                    for (int i = 0; i < toAdd; i++) {
                        n.getOutputs().add(createNewPin(n));
                    }
                }
                n.computeSize();
