 ...
 * for inheritance and should not be subclassed. In particular, self-use by overridable methods is
...
public class JSONArray implements List<Object> {
    ...
    public boolean add(Object value) {
        return this.values.add(JSONObject.wrap(value));
    }

    @Override
    public boolean addAll(Collection<? extends Object> list) {
        this.values.ensureCapacity(this.values.size() + list.size());
        for (Object o : list) {
            add(o);
        }
        return false;
     ...
     */
    void checkedAdd(Object value) throws JSONException {
     ...
     * 
     * @param index
     * @return true if this array has no value or its value is null or {@link JSONObject#NULL}
     */
     ...
     * 
     * @param index
     * @return the value at {@code index} if it exists
     *
    ...
    /**
     * Returns the value at {@code index} if it exists and is a double or can be coerced to a
     * double.
     * 
     * @param index
     * @return the value at {@code index} if it exists
    ...
    /**
     * Returns the value at {@code index} if it exists and is a double or can be coerced to a
     * double. Returns {@code NaN} otherwise.
     * 
     * @param index
     * @return the value at {@code index} if it exists
     */
    ...
    /**
     * Returns a new object whose values are the values in this array, and whose names are the
     * values in {@code names}. Names and values are paired up by index from 0 through to the
     * shorter array's length. Names that are not strings will be coerced to strings. This method
     * returns null if either array is empty.
     * 
     * @param names
    ...
    public JSONObject toJSONObject(JSONArray names) throws JSONException {
        ...
        int length = Math.min(names.size(), this.values.size());
        if (length == 0) {
            return null;
        }
        for (int i = 0; i < length; i++) {
    ...
    public void copyValuesTo(List<Object> col) {
        col.addAll(this.values);
    }
